...
const handleSubmit = ({ editCommentMutation, commentId, editedMessage, isPublic, setErrors, setEditMode }) => async event => {
  event.preventDefault();
  const errors = validateMessage({ message: editedMessage });
  setErrors(errors);
  if (errors) { return; }
  editCommentMutation({ commentId, message: editedMessage, isPublic })
...
const enhanced = compose(
  withState('editedMessage', 'setMessage', ({ message }) => message),
